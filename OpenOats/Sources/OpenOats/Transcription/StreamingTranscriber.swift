...
final class StreamingTranscriber: @unchecked Sendable {
            ...
            while vadBuffer.count - vadReadIndex >= Self.vadChunkSize {
                ...
                do {
                    ...
                    if let event = result.event {
                        ...
                        case .speechStart:
                            if !wasSpeaking {
                                ...
                                Log.streaming.info("[\(self.speaker.storageKey, privacy: .public)] speech start")
                            }
                    ...
                    if endedSpeech {
                        ...
                        Log.streaming.info("[\(self.speaker.storageKey, privacy: .public)] speech end, samples=\(speechSamples.count, privacy: .public)")
                    ...
                    Log.streaming.error("VAD error: \(error.localizedDescription, privacy: .public)")
                }
            }
        }
    ...
    private func transcribeSegment(_ samples: [Float]) async {
        do {
            ...
            Log.streaming.info("[\(self.speaker.storageKey, privacy: .public)] transcribed: \(text.prefix(80), privacy: .private)")
            // Store trailing words for cross-segment context
            let words = text.split(separator: " ")
            previousContext = words.suffix(Self.contextWordCount).joined(separator: " ")
            onFinal(text)
        } catch {
            Log.streaming.error("ASR error: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private func extractSamples(_ buffer: AVAudioPCMBuffer) -> [Float]? {
        ...
        if let error {
            Log.streaming.error("Resample error: \(error.localizedDescription, privacy: .public)")
            return nil
        }
